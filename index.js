...
function search() { // Repete of the above code for single run
    ...
    var inputs = document.getElementsByTagName('input'); // This should be less genral but it works for now
    ...
    twits = document.getElementsByClassName('twit'); // get all the twits
    var x = twits.toLowercase();
    document.write(x);

    for (i = 0; i < twits.length; i++) { // Fixing the whitelist
        if (twits.toLowerCase().includes(p1)) { // See if the string is included in the twit textContent
            console.log("Contains: ", p1, " ", twits[i]); // Log all members that have the input
            whiteList.push(twits[i]); // Then add them to the whiteList
        }
    }
...
function makeTwit(in1, in2) {
    ...
    t4.appendChild(t5);
    var t6 = document.createElement('p'); // Create the author
    t6.classList.add('twit-author');
